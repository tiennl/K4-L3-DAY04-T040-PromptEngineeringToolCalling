...
| Nguyễn Khánh Linh | 2A202602409 | klinhnguyen2012 | UI & Transcript | PR #4 — `starter_v0/chat.py` (tool trace UI), `starter_v0/tests/test_chat.py`, `starter_v0/transcripts/ticket-cancellation-v0.transcript.json`, `starter_v0/transcripts/ticket-modification-failed-v0.transcript.json` |
...
- [x] Đặt giả thuyết → sửa `artifacts/system_prompt.md` và/hoặc `artifacts/tools.yaml` → chạy v1 (case_accuracy 0.6667 → 0.7667)
- [x] Lặp lại cho v2, v3 (mỗi vòng 1 giả thuyết rõ) — v2: 0.9000 (đỉnh), v3: 0.8333 (regress 4 case, xem `artifacts/REPORT.md` B2)
- [x] Ghi mọi lần chạy vào `artifacts/version_log.csv` (thay đổi, lý do, hash, metric trước/sau, đường dẫn run)
...
- [x] Viết đúng 10 case vào `data/eval_group.json` (5 một lượt + 5 nhiều lượt), có đầu ra kỳ vọng
- [x] Chạy case nhóm cùng các version của Người 1 khi có — `runs/v0-v3_B_group_openai_*.json`
...
- [x] Chạy/chỉnh `chat.py` sao cho UI hiện rõ tool được gọi, input, kết quả/lỗi và version đang chạy (PR #4)
- [x] Test UI với cả case thường và case đa lượt (thiếu info, sửa/hủy)
- [x] Lưu transcript của các hội thoại bắt buộc (hỏi lại, xác nhận, hủy) — `transcripts/ticket-cancellation-v0.transcript.json`, `transcripts/ticket-modification-failed-v0.transcript.json`
...
- [x] Theo dõi tiến độ 3 người kia, tổng hợp số liệu trước/sau vào `artifacts/REPORT.md`
- [x] Viết phần cách chạy, giới hạn còn lại, liên kết evidence (run file, version log, transcript)
